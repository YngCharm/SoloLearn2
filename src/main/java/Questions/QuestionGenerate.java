...
public class QuestionGenerate {
    ...
    int numberOfAnswer = random.nextInt(max + min) + min;
    ...
    public void showQuestion() {
        ...
        int min = 0;
        int max = 9;
        int numberOfQuestions = min + (int) (Math.random() * max);

        System.out.println(listQuestion.get(numberOfQuestions).question + "\n"
                + Arrays.toString(listQuestion.get(numberOfQuestions).answer));

        listQuestion.remove(numberOfQuestions);

        correctAnswer = listQuestion.get(numberOfQuestions).correctAnswer;
    }
}
